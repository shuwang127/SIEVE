...
[MC, MK] = mutualcorread( 'passenger1.wav', L, d, S0, P_th );
LB = (1/sqrt(2)) * ones( size(MC,1), 1 );
data = [data; MC];
mark = [mark; MK];
label = [label; LB];
data_test = [data_test; data(3313, :)];
label_test = [label_test; label(3313, :)];
% 6067
...
[MC, MK] = mutualcorread( 'passenger2.wav', L, d, S0, P_th );
LB = (-1/sqrt(2)) * ones( size(MC,1), 1 );
data = [data; MC];
mark = [mark; MK];
label = [label; LB];
...
[MC, MK] = mutualcorread( 'passenger3.wav', L, d, S0, P_th );
